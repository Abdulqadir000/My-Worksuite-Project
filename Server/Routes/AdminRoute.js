...
router.post("/adminsignup", async (req, res) => {
  try {
    ...
    if (!email || !password) {
      return res
        .status(400)
        .json({ signupStatus: false, error: "Email aur password required hain" });
    }
    ...
    if (!emailRegex.test(email)) {
      return res
        .status(400)
        .json({ signupStatus: false, error: "Invalid email format" });
    }

    // check if user already exists
    const exist = await db.query("SELECT id FROM admin WHERE email = $1", [
      email,
    ]);
    if (exist.rows.length > 0) {
      return res
        .status(409)
        .json({ signupStatus: false, error: "Admin pehle se maujood hai" });
    }
    ...
    // create jwt and set cookie (optional: same as login)
    const token = jwt.sign(
      { role: "admin", email: user.email, id: user.id },
      process.env.JWT_SECRET,
      { expiresIn: "1d" }
    ...
    return res
      .status(201)
      .json({ signupStatus: true, message: "Admin created", id: user.id });
  } catch (err) {
    console.error("Signup Error:", err);
    return res
      .status(500)
      .json({ signupStatus: false, error: "Server error" });
  }
});
